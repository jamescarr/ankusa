defmodule Hook.Sink.RabbitMQ do
  @moduledoc """
  ...
  `Hook.BlobStore` (reusing the existing S3/GCS/LocalFS adapters — no new
  ...
  ## opts
    ...
    * `:routing_key`       — a static string, or a 1-arity fun `(Envelope.t() -> String.t())`;
                              default `"hook.\#{source_id}"`
    * `:inline_max_bytes`  — default `8_192`
    * `:blob_store`        — `{module, opts}` implementing `Hook.BlobStore`; default:
  ...
  alias Hook.Envelope
  alias Hook.Sink.RabbitMQ.Connection
  ...
  defp ensure_started(instance, exchange, opts) do
    name = Hook.via(instance, {:rabbitmq_conn, exchange})
    ...
    case DynamicSupervisor.start_child(Hook.Sink.RabbitMQ.Supervisor, child) do
      {:ok, _pid} -> {:ok, name}
  ...
  defp resolve_blob_store(ctx, opts) do
    case Keyword.get(opts, :blob_store) do
      nil ->
        %Hook.Config{storage: %{blob_store: bs}} = Hook.config(ctx.instance)
        bs
  ...
  defp default_routing_key(env), do: "hook.#{env.source_id}"
end
