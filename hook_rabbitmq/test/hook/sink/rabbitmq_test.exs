defmodule Hook.Sink.RabbitMQTest do
  @moduledoc """
  ...
  alias Hook.{Envelope, UUIDv7}
  alias Hook.Sink.RabbitMQ

  @amqp_url "amqp://guest:guest@localhost:5673"

  setup do
    instance = :"rmq_#{System.unique_integer([:positive])}"
    exchange = "hook.test.#{System.unique_integer([:positive])}"
  ...
  test "inline payload publishes a small message the consumer can decode", %{
    ...
    assert meta.routing_key == "hook.src"
  ...
  test "a fat payload is offloaded to the blob store and the message carries a pointer", %{
    ...
    blob_dir = Path.join(System.tmp_dir!(), "hook_rmq_blob_#{System.unique_integer([:positive])}")
    on_exit(fn -> File.rm_rf(blob_dir) end)
    Hook.put_config(Hook.Config.new(instance: inst, data_dir: blob_dir))
    ...
    assert {:ok, ^body} = Hook.BlobStore.LocalFS.get(inst, key, [])
  end
