defmodule Hook.TestHelpers do
  ...
  @doc """
  Build an isolated `%Hook.Config{}` with a fresh temp `data_dir` (auto-cleaned)
  and an ephemeral HTTP port. Pass `overrides` as keyword to `Hook.Config.new/1`.
  """
  def test_config(overrides \\ []) do
    inst = Keyword.get(overrides, :instance, unique_instance())
    dir = Path.join(System.tmp_dir!(), "hook_#{inst}_#{System.unique_integer([:positive])}")
  ...
  def put_config(config), do: Hook.put_config(config)

  @doc "Build a raw ingest request map for `Hook.Edge.Ingest`/router."
