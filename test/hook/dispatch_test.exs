defmodule Hook.DispatchTest do
  use ExUnit.Case, async: false

  alias Hook.{Config, Envelope, WAL}
  alias Hook.Dispatch.{DLQ, Pipeline}
  ...
  defp start(sinks, opts) do
    ...
    base = [
      ...
      source_store: {Hook.SourceStore.Static, sources: %{"src1" => %{sinks: sinks}}}
    ]
  ...
  test "dead-letters after exhausting the retry policy" do
    %{inst: inst, config: config} =
      start([{AlwaysFail, []}],
        max_sleep_ms: 5,
        dispatch: %{retry: {Hook.RetryPolicy.Exponential, max_attempts: 2}}
      )
  ...
  test "replay re-delivers dead-lettered hooks through the source sinks" do
    ...
    replay_config =
      Config.new(
        ...
        source_store:
          {Hook.SourceStore.Static,
           sources: %{"src1" => %{sinks: [{CapturingSink, pid: self()}]}}}
      )

    Hook.put_config(replay_config)

    assert Hook.Dispatch.replay(inst, source_id: "src1") == 1
    assert_receive {:delivered, id, 1}
    assert id == env.id
  end
end
