defmodule Hook.Codec.RawTest do
  use ExUnit.Case, async: true

  alias Hook.Codec.Raw
...
defmodule Hook.StorageTest do
  use ExUnit.Case, async: false

  alias Hook.{Config, Envelope}
  alias Hook.Storage
  alias Hook.Storage.{Compactor, Index}

  setup do
    inst = :"t#{System.unique_integer([:positive])}"
    dir = Path.join(System.tmp_dir!(), "hook_#{inst}")
    ...
    config =
      Config.new(
        ...
        source_store: {Hook.SourceStore.Static, sources: %{"acme" => %{}}},
        # disable the interval auto-tick so `tick/1` fully controls the test
        storage: %{interval_ms: 0}
      )

    Hook.put_config(config)
    start_supervised!({Hook.WAL.DiskLog, instance: inst, config: config})
  ...
  defp commit!(inst, envelopes) do
    ...
    {:ok, results} = Hook.WAL.append(inst, records)
  ...
  test "tick compacts WAL records into one segment, indexes them, and truncates",
    ...
    :ok = Hook.WAL.put_cursor(inst, :dispatch, last_seq)
    assert Hook.WAL.stats(inst).records == 4

    assert {:ok, 1} == Compactor.tick(inst)

    # exactly one immutable segment written
    assert [segment_key] = Hook.BlobStore.list(inst, "seg")
    ...
    assert Hook.WAL.stats(inst).records == 0
  ...
  test "compaction never truncates past the dispatch cursor", %{inst: inst, config: config} do
    ...
    :ok = Hook.WAL.put_cursor(inst, :dispatch, first_seq)
