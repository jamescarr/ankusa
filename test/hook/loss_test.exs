  ...
  test "every acked hook survives a hard crash of the whole instance" do
    config =
      test_config(
        roles: [:edge],
        source_store:
          {Hook.SourceStore.Static, sources: %{"load" => [verifier: {Hook.Verifier.None, []}]}},
        batcher: %{partitions: 4, max_batch: 64, max_delay_ms: 5, max_queue: 100_000}
      )

    pid = start_supervised!({Hook.Instance, config})
    ...
    start_supervised!({Hook.WAL.DiskLog, instance: config.instance, config: config})
