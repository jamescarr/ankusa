defmodule Hook.WAL.DiskLogTest do
  use ExUnit.Case, async: false

  import Hook.TestHelpers
  alias Hook.{Envelope, WAL}

  setup do
    config = test_config()
    put_config(config)
    pid = start_supervised!({Hook.WAL.DiskLog, instance: config.instance, config: config})
  ...
  test "recovers committed records and drops a torn trailing frame across restart", %{
    ...
    :ok = stop_supervised({Hook.WAL.DiskLog, inst})

    # simulate a crash mid-write: append a partial (never-fsync'd) frame
    path = Path.join([config.data_dir, to_string(inst), "wal", "hook.wal"])
