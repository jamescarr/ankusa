defmodule Hook.RouteResolverTest do
  use ExUnit.Case, async: false

  import Hook.TestHelpers
  alias Hook.Edge.Router
  alias Hook.{Route, RouteResolver, WAL}
    defp start(resolver, sources) do
      config =
        test_config(
          roles: [:edge],
          route_resolver: resolver,
          source_store: {Hook.SourceStore.Static, sources: sources}
        )
  ...
  describe "integration through the edge" do
    test "the URL tenant is threaded onto the envelope and scopes dedup" do
      config =
        start({Hook.RouteResolver.TenantPath, []}, %{
          "stripe" => [dedup: {Hook.DedupKey.Stripe, []}]
        })
    ...
    test "an unresolvable URL shape is a 404" do
      config = start({Hook.RouteResolver.TenantPath, []}, %{"stripe" => []})
