defmodule Hook.VerifierTest do
  ...
  describe "Hook.Verifier.None" do
    test "always accepts" do
      assert Hook.Verifier.None.verify(env([], "anything"), []) == :ok
      assert Hook.Verifier.None.verify(env([{"x", "y"}], ""), secret: "ignored") == :ok
    end
  end

  describe "Hook.Verifier.StandardWebhooks" do
    setup do
    ...
    test "accepts a valid signature", %{key: key, secret: secret} do
      ...
      assert Hook.Verifier.StandardWebhooks.verify(
    ...
    test "rejects a tampered body", %{key: key, secret: secret} do
      ...
      assert {:error, :no_match} =
               Hook.Verifier.StandardWebhooks.verify(
    ...
    test "rejects missing headers", %{secret: secret} do
      assert {:error, :missing_headers} =
               Hook.Verifier.StandardWebhooks.verify(env([], "body"), secret: secret)
    end
  end

  describe "Hook.Verifier.Stripe" do
    setup do
    ...
    test "accepts a valid signature", %{secret: secret} do
      ...
      assert Hook.Verifier.Stripe.verify(
    ...
    test "rejects a missing header", %{secret: secret} do
      assert {:error, :missing_signature} =
               Hook.Verifier.Stripe.verify(env([], "body"), secret: secret)
    end
  end

  describe "Hook.Verifier.GitHub" do
    setup do
    ...
    test "accepts a valid signature", %{secret: secret} do
      ...
      assert Hook.Verifier.GitHub.verify(
    ...
    test "rejects a missing header", %{secret: secret} do
      assert {:error, :missing_signature} =
               Hook.Verifier.GitHub.verify(env([], "body"), secret: secret)
    end
  end
end
