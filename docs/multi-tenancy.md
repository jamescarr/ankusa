...
where every customer gets their own catch URL, and a Zapier-style product
...
Zapier-style opaque token (`POST /hooks/catch/:app_id/:token`), a
...
fixed at boot. A real multi-tenant SaaS or Zapier-style product needs a
