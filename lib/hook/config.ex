defmodule Hook.Config do
  ...
  defstruct instance: :default,
            data_dir: "./data",
            # which OTP roles boot in this node (HOOK_ROLES in production)
            roles: [:edge, :dispatch, :storage],
            # Bandit edge
            port: 4000,
            # {module, opts} implementing Hook.RouteResolver (URL scheme → identity)
            route_resolver: {Hook.RouteResolver.Path, []},
            max_body_bytes: 8_000_000,
            # {module, opts} implementing Hook.SourceStore
            source_store: {Hook.SourceStore.Static, sources: %{}},
            # {module, opts} implementing Hook.WAL
            ...
            dispatch: %{
              poll_ms: 200,
              batch: 128,
              retry: {Hook.RetryPolicy.Exponential, []}
            },
            # segment compaction
            storage: %{
  ...
  @doc """
  Build a `%Hook.Config{}` from a keyword list, deep-merging the map-valued
  ...
  def new(opts \\ []) do
    ...
    Enum.reduce(opts, base, fn {k, v}, acc ->
      cond do
        ...
        true ->
          raise ArgumentError, "unknown Hook.Config key: #{inspect(k)}"
      end
    end)
  end
