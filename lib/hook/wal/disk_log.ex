defmodule Hook.WAL.DiskLog do
  ...
  `payload` is a serialized `Hook.Envelope`; `crc32` covers the payload. Replay
  ...
  def start_link(opts) do
    ...
    GenServer.start_link(__MODULE__, opts, name: Hook.via(instance, :wal))
  end
  ...
  def init(opts) do
    ...
    File.mkdir_p!(dir)
    path = Path.join(dir, "hook.wal")

    dedup = :ets.new(:hook_wal_dedup, [:set, :protected, read_concurrency: true])
    index = :ets.new(:hook_wal_index, [:ordered_set, :protected])
    ...
    Logger.info(
      "[hook] DiskLog WAL at #{path}: recovered #{:ets.info(index, :size)} record(s), " <>
