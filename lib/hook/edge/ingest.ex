defmodule Hook.Edge.Ingest do
  ...
  alias Hook.{Envelope, Source, Verification}
  alias Hook.Edge.{Batcher, BatcherSupervisor, Quarantine}
  ...
  def ingest(instance, %{source_id: source_id} = req) do
    Hook.Telemetry.span([:ingest], %{instance: instance, source_id: source_id}, fn ->
      result =
        case Hook.SourceStore.fetch(instance, source_id) do
  ...
  defp verify(instance, %Source{verifier: {mod, opts}} = source, env) do
    outcome =
      Hook.Telemetry.span([:verify], %{instance: instance, source_id: source.id}, fn ->
  ...
  defp dedup_key(instance, env) do
    {mod, opts} =
      case Hook.SourceStore.fetch(instance, env.source_id) do
        {:ok, %Source{dedup: dedup}} -> dedup
        :error -> {Hook.DedupKey.Rules, []}
      end
