defmodule Hook.Edge.Quarantine do
  @moduledoc """
  ...
  def start_link(opts) do
    ...
    GenServer.start_link(__MODULE__, opts, name: Hook.via(instance, :quarantine))
  end
  ...
  def put(instance, env, reason) do
    GenServer.call(Hook.via(instance, :quarantine), {:put, env, reason})
  end

  @doc "List recent quarantined entries (newest first)."
  @spec recent(atom()) :: [map()]
  def recent(instance), do: GenServer.call(Hook.via(instance, :quarantine), :recent)
  ...
  def handle_call({:put, env, reason}, _from, state) do
    ...
    if state.tokens >= 1.0 do
      ...
      Hook.Telemetry.emit([:quarantine, :rate_limited], %{}, %{
