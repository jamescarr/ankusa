defmodule Hook.Edge.Batcher do
  @moduledoc """
  ...
  def start_link(opts) do
    ...
    GenServer.start_link(__MODULE__, opts, name: Hook.via(instance, {:batcher, partition}))
  end
  ...
  @spec commit(atom(), non_neg_integer(), WAL.entry(), timeout()) ::
          {:committed, Hook.Envelope.t()} | {:duplicate, non_neg_integer()} | {:error, :overload}
  def commit(instance, partition, record, timeout \\ 15_000) do
    GenServer.call(Hook.via(instance, {:batcher, partition}), {:enqueue, record}, timeout)
  end
  ...
  def handle_call({:enqueue, record}, from, state) do
    cond do
      state.count >= state.max_queue ->
        Hook.Telemetry.emit([:load_shed], %{queue: state.count}, %{instance: state.instance})
