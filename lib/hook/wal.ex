defmodule Hook.WAL do
  ...
  A record is `%{envelope: Hook.Envelope.t()}`; the `dedup_key` is read from the
  ...
  defp apply_mod(instance, fun, args) do
    %Config{wal: {mod, _opts}} = Hook.config(instance)
    apply(mod, fun, [Hook.via(instance, :wal) | args])
  end
end
