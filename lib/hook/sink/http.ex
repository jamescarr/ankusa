defmodule Hook.Sink.Http do
  ...
  (falling back to `application/octet-stream`). Identity headers `x-hook-id`,
  `x-hook-source`, and `x-hook-seq` are always added. A `2xx` response is `:ok`;
  ...
  def deliver(env, _ctx, opts) do
    ...
    headers =
      [
        {~c"x-hook-id", to_charlist(env.id)},
        {~c"x-hook-source", to_charlist(env.source_id)},
        {~c"x-hook-seq", to_charlist(to_string(env.seq))}
