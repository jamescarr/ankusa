defmodule Hook.Storage.Compactor do
  @moduledoc """
  ...
  blob store, appends the per-record rows to `Hook.Storage.Index`, and advances
  ...
  def start_link(opts) do
    ...
    GenServer.start_link(__MODULE__, opts, name: Hook.via(instance, :compactor))
  end
  ...
  def tick(instance), do: GenServer.call(Hook.via(instance, :compactor), :tick)

  @impl true
  def init(opts) do
    instance = Keyword.fetch!(opts, :instance)
    %Config{} = config = Keyword.fetch!(opts, :config)
    cursor = Hook.WAL.get_cursor(instance, :compactor)
  ...
  defp compact(%{instance: instance, config: config, cursor: cursor} = state) do
    case Hook.WAL.read(instance, cursor, @read_limit) do
      [] ->
        {0, state}
      ...
      envelopes ->
        ...
        :ok = Hook.BlobStore.put(instance, key, segment)
        ...
        :ok = Hook.WAL.put_cursor(instance, :compactor, last_seq)

        # never truncate past what dispatch has consumed — at-least-once
        dispatch_seq = Hook.WAL.get_cursor(instance, :dispatch)
        :ok = Hook.WAL.truncate_through(instance, min(last_seq, dispatch_seq))
