defmodule Hook.WAL.Postgres do
  @moduledoc """
  Shared, multi-node `Hook.WAL` backed by Postgres. This is the adapter that
  ...
  `Hook.via(instance, :wal)` name DiskLog would use), and coordination between
  ...
  `COMMIT`, i.e. one fsync, per batch — matching the core invariant):
    ...
    1. **Claim dedup keys.** A single `INSERT ... ON CONFLICT DO NOTHING`
       against `hook_wal_dedup`, batched via `unnest/1`. Postgres takes a
    ...
    2. **Insert winners.** Rows that either had no dedup key or won their
       claim are inserted into `hook_wal` (again batched via `unnest/1`),
    ...
    3. **Resolve losers' seq.** For rows that lost their dedup claim, one
       lookup joins `hook_wal_dedup` back to `hook_wal` by `event_id` to find
  ...
  ## Config
      ...
      config :hook,
        wal: {Hook.WAL.Postgres, hostname: "localhost", port: 5433,
  ...
  alias Hook.Envelope
  alias Hook.WAL.Postgres.Migration
  ...
  def read(server, after_seq, limit) do
    ...
    %Postgrex.Result{rows: rows} =
      Postgrex.query!(
        server,
        "SELECT seq, envelope FROM hook_wal WHERE instance = $1 AND seq > $2 ORDER BY seq LIMIT $3",
  ...
  def get_cursor(server, name) do
    ...
    case Postgrex.query!(
           server,
           "SELECT seq FROM hook_wal_cursors WHERE instance = $1 AND name = $2",
  ...
  def put_cursor(server, name, seq) do
    ...
    Postgrex.query!(
      server,
      """
      INSERT INTO hook_wal_cursors (instance, name, seq) VALUES ($1, $2, $3)
  ...
  def truncate_through(server, seq) do
    instance = instance_key(server)
    # `hook_wal_dedup` is untouched — dedup keys survive truncation.
    Postgrex.query!(server, "DELETE FROM hook_wal WHERE instance = $1 AND seq <= $2", [
      instance,
      seq
    ])
  ...
  def stats(server) do
    ...
    %Postgrex.Result{rows: [[records, min_seq, max_seq, bytes]]} =
      Postgrex.query!(
        server,
        """
        SELECT count(*), min(seq), max(seq), COALESCE(pg_total_relation_size('hook_wal'), 0)
        FROM hook_wal WHERE instance = $1
        """,
        [instance]
      )

    %Postgrex.Result{rows: cursor_rows} =
      Postgrex.query!(server, "SELECT name, seq FROM hook_wal_cursors WHERE instance = $1", [
        instance
      ])
      ...
      dedup_rows ->
        %Postgrex.Result{rows: claimed} =
          Postgrex.query!(
            conn,
            """
            INSERT INTO hook_wal_dedup (instance, tenant_id, source_id, dedup_key, event_id)
  ...
  defp insert_winners(conn, winners) do
    %Postgrex.Result{rows: inserted} =
      Postgrex.query!(
        conn,
        """
        INSERT INTO hook_wal (event_id, instance, tenant_id, source_id, dedup_key, envelope)
  ...
  # Fills in `hook_wal_dedup.seq` for the winners that claimed a dedup key,
  # now that Postgres has assigned their real `hook_wal.seq`. Keeps the dedup
  # ledger self-contained (see the migration moduledoc) instead of relying on
  # a join back to `hook_wal`, which would stop finding a key the moment its
  ...
  # The facade always calls with `Hook.via(instance, :wal)` — pattern-match
  ...
  defp instance_key({:via, Registry, {Hook.Registry, {instance, :wal}}}), do: to_string(instance)
end
